...
int main()
{
    ...
    color target_average = make_tuple(0.0, 0.0, 0.0);
    for (const auto& [c, m, y] : target_colors)
    {
        get<0>(target_average) += c;
        get<1>(target_average) += m;
        get<2>(target_average) += y;
    }
    get<0>(target_average) /= H;
    get<1>(target_average) /= H;
    get<2>(target_average) /= H;
    ...
    for (size_t iter = 0; iter < ITERATION; ++iter)
    {
        ...
        {
            if (clusters[i].empty()) {
                // クラスタが空の時は、centerをtarget_averageに設定
                cluster_centers[i] = target_average;
                continue;
            }
    ...
    for (size_t i = 0; i < H; ++i)
    {
        ...
        {
            double error = 0;
            if (!color_indices_of_clusters[j].empty()) {
                error = calc_error(target_colors[i], target_colors[color_indices_of_clusters[j].back()]);
            }
            else {
                error = calc_error(target_colors[i], clusters_centers[j]);
            }
