...
public:
	...
	void allocate(std::vector<std::uint32_t> reservations)
	{
		...
		while (allocating_index >= 0) {
			...
			fitting_index = -1;
			std::int32_t size_error = ((std::int32_t)1) << 31;
			for (std::size_t i = 0; i < left.size(); i++) {
				std::uint32_t available_size = left[i]->getAvailableSize();
				std::int32_t error = available_size - reservations[allocating_index];
				if ((size_error < 0 && size_error < error) || (size_error >= 0 && size_error > error && error >= 0))
				{
					fitting_index = i;
					size_error = error;
				}
			...
			if (left[fitting_index]->getAvailableSize() > 0) {
				areas[allocating_index] = left[fitting_index]->allocate(reservations[allocating_index]);
				allocating_index--;
				continue;
			}
...
int main()
{
	...
	for (std::uint64_t i = 0; i < D; i++)
	{
		...
		{
			std::cin >> reservations[i][j];
		}
	}

	//std::uint32_t reach_half_max = 0;
	//for (std::uint64_t i = 0; i < D; i++)
	//{
